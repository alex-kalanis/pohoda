...
final class DtoDIFactory implements DtoFactoryInterface
{
    ...
    public function getDto(string|object $name): AbstractDto
    {
        ...
        if (!$this->container->has($className)) {
            throw new DomainException('Dto class does not exists: ' . $name);
        }

        try {
            $instance = $this->container->get($className);
        } catch (ContainerExceptionInterface $ex) {
            throw new DomainException('Dto class initialization failed: ' . $name, 0, $ex);
        }
        ...
        throw new DomainException('Dto class is not an instance of AbstractDto: ' . $name);
    }
}
